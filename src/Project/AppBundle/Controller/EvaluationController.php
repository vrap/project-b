...
class EvaluationController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        if(in_array('ROLE_MANAGER', $userRoles)){
            $entities = $em->getRepository('ProjectAppBundle:Evaluation')->findAllUnvalidatedByPromotion($this->get('session')->get('promotion'));
        }else{
        ...
        if ($this->get('session')->get('promotion') == $promotion->getId()) {
            ...
            foreach ($studentNotes as $note) {
                ...
                foreach ($studentCriterions as $criterion) {
                    $notes[$studentId]['details'][$criterion->getId()] = $criterion;
                }
            }
